...
const translations = {
  ...
  zh: {
    ...
    reply_button: "回复",
    no_comments: "暂无评论。",
    add_comment_title: "添加评论",
    publish_button: "发布",
    login_to_comment: "你必须",
    login_link: "登录",
    to_comment: "才能评论。",
    form_label_texto: "内容",
    form_placeholder_texto: "写下你的评论...",
    blog_title: "个人博客",
    no_entries: "没有找到条目。",
    video_not_supported: "您的浏览器不支持视频标签。",
    audio_not_supported: "您的浏览器不支持音频标签。",
    download_attachment_link: "下载附件",
    login_title: "登录",
    login_button: "登录",
...
// Función para construir o reconstruir los comentarios con textos traducidos
function actualizarComentariosIdioma(lang) {
  ...
  document.querySelectorAll('.comentario-texto').forEach(span => {
    ...
    textoCompleto = textoCompleto.replace(/Leer más...|Leer menos|Read more...|Read less|阅读更多...|阅读更少/gi, '').trim();

    if (textoCompleto.length > limite) {
      const visible = textoCompleto.slice(0, limite);
      const oculto = textoCompleto.slice(limite);
      const leerMas = translations[lang]?.Read_more || "Leer más...";
...
// Inicializar traducción con idioma guardado o español por defecto
